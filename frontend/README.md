...
| `swag-collection.json` | The live `Swag1155` clone (`ETHCALI-SWAG-2026`) on Base, read from `scs-ethcali/deployments/base-latest.json`. | `config/chains.ts` (`features.swag`), `config/constants.ts` (`SWAG_COLLECTION_BASE`) |
...
`config/chains.ts` turns it into a per-chain `features` map that every page
reads through `chainsFor('faucet' | 'identity' | 'donations' | 'swag' | …)`.
To see the current matrix:
