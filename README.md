...
- Swag store: one ERC-1155 tokenId per design on Base, paid in USDC on chain or by card through Shopify (the NFT is then claimed with a server-signed voucher)
...
│   ├── useActiveWallet.ts, useRequireChain.ts, useAdminStatus.ts, useChainBalances.ts,
...
├── config/                  # chains.ts (THE chain registry: chains, RPCs, tokens, per-chain contracts + features),
│                            # constants.ts (non-chain: SWAG_COLLECTION_BASE, ENS_CONFIG, decimals)
├── utils/, types/, styles/
...
- 17 designs, one ERC-1155 tokenId each, on Base only; size is chosen at checkout, not on chain
...
| `swag-seed-supabase.mjs <catalogue> [--collection 0x…]` | Upserts `swag_products`, `swag_variants` (Base) and `swag_shopify_variants` from the catalogue |
...
There is no global network selector. Each feature picks its chain in-page from the
chains it is actually deployed on (`chainsFor(feature)` in
[`config/chains.ts`](config/chains.ts)), reads with that chain's own client, and asks
the wallet to switch only when a transaction is about to be signed
(`hooks/useRequireChain.ts`). Which contract exists on which chain comes from the
generated `frontend/addresses.json`; deploying a contract to a new chain and running
`npm run sync:contracts` is what turns that feature on there. Adding a chain is one
entry in `CHAIN_DEFS`.
