  ...
  if (isLoadingVariant) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
        <div className="text-content-primary">Loading...</div>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-card border border-line-hairline bg-surface-slab p-6 ">
        <div className="flex items-start justify-between mb-6">
          <div>
            <h2 className="text-xl font-semibold text-content-primary">Edit Token #{tokenId}</h2>
            <p className="text-sm text-content-faint font-mono">{contractAddress.slice(0, 10)}...</p>
          </div>
        ...
        {/* Tabs */}
        <div className="flex gap-2 mb-6 border-b border-line-hairline overflow-x-auto">
          <button
            onClick={() => setActiveTab('variant')}
            className={`px-4 py-2 text-sm font-medium transition whitespace-nowrap ${
              activeTab === 'variant' ? 'text-eth-blue-text border-b-2 border-eth-blue' : 'text-content-muted hover:text-content-secondary'
            }`}
          >
            Variant Info
          </button>
          <button
            ...
            className={`px-4 py-2 text-sm font-medium transition whitespace-nowrap ${
              activeTab === 'metadata' ? 'text-eth-blue-text border-b-2 border-eth-blue' : 'text-content-muted hover:text-content-secondary'
            }`}
          >
            Metadata
          </button>
          <button
            ...
            className={`px-4 py-2 text-sm font-medium transition whitespace-nowrap ${
              activeTab === 'royalties' ? 'text-eth-blue-text border-b-2 border-eth-blue' : 'text-content-muted hover:text-content-secondary'
            }`}
          >
            Royalties
          </button>
          <button
            ...
            className={`px-4 py-2 text-sm font-medium transition whitespace-nowrap ${
              activeTab === 'discounts' ? 'text-eth-blue-text border-b-2 border-eth-blue' : 'text-content-muted hover:text-content-secondary'
            }`}
          >
            Discounts
          </button>
        ...
        {activeTab === 'variant' && (
          <div className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <label className="block text-sm text-content-muted">Max Supply</label>
                <input type="number" min="0" value={maxSupplyInput}
                  onChange={(e) => setMaxSupplyInput(e.target.value)}
                  className="w-full rounded-control border border-line-hairline bg-surface-inset p-3 text-content-primary focus:border-eth-blue focus:outline-none"
                  disabled={isSubmitting} />
                {variant && <p className="text-xs text-content-faint">{Number(variant.minted)} already minted</p>}
              </div>
            </div>

            <div className="space-y-2">
              <label className="block text-sm text-content-muted">Metadata URI</label>
              <input type="text" value={uriInput}
                ...
                disabled={isSubmitting} />
              <p className="text-xs text-content-faint">Use the Metadata tab for a rich editing experience</p>
            </div>
            ...
            <div className="flex items-center justify-between rounded-control border border-line-hairline bg-surface-inset p-4">
              <div>
                ...
                <p className="text-xs text-content-faint">{activeInput ? 'Available for purchase' : 'Not available'}</p>
              </div>
              <button type="button" onClick={() => setActiveInput(!activeInput)}
                className={`relative h-6 w-11 rounded-full transition-colors ${activeInput ? 'bg-eth-blue' : 'bg-surface-ridge'}`}
                disabled={isSubmitting}>
                <span className={`absolute top-0.5 h-5 w-5 rounded-full bg-surface-paper transition-transform ${activeInput ? 'translate-x-5' : 'translate-x-0.5'}`} />
              </button>
            </div>
        ...
        {activeTab === 'metadata' && (
          <div className="space-y-5">
            {metadataLoading ? (
              ...
              <div className="rounded-control border border-signal-reverted/40 bg-signal-reverted/10 p-4">
                <p className="text-sm text-signal-reverted">{metadataError}</p>
                <p className="text-xs text-content-faint mt-2">You can still fill in new metadata below.</p>
              </div>
            ) : null}

            {/* Image */}
            <div className="space-y-2">
              <label className="block text-sm text-content-muted">Image</label>
              <div
                onClick={() => !isSubmitting && fileInputRef.current?.click()}
                onDrop={handleDrop}
                onDragOver={e => e.preventDefault()}
                className={`relative flex flex-col items-center justify-center rounded-card border-2 border-dashed p-4 cursor-pointer transition ${
                  imagePreviewUrl ? 'border-eth-blue/50 bg-surface-inset/50' : 'border-line-strong bg-surface-inset/30 hover:border-line-strong'
                }`}
              >
                {imagePreviewUrl ? (
                  <img src={imagePreviewUrl} alt="Preview" className="max-h-40 rounded-control object-contain" />
                ) : (
                  <>
                    <svg className="h-8 w-8 text-content-faint mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                    <p className="text-sm text-content-muted">Click or drag to upload new image</p>
                  </>
                )}
                {newImagePreview && (
                  <span className="absolute top-2 right-2 rounded-chip bg-eth-blue/20 px-2 py-0.5 text-xs text-eth-blue-text">New</span>
                )}
                <input
                  ref={fileInputRef}
                  type="file"
              ...
              <p className="text-xs text-content-faint">Click to replace. Max 5MB, PNG/JPG/WebP</p>
            </div>

            {/* Name & Description */}
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="block text-sm text-content-muted">Name</label>
                <input
                  type="text"
                  value={metadataName}
            ...
            <div className="space-y-3">
              <p className="text-sm font-medium text-content-secondary">Traits</p>
              <div className="space-y-2">
                <label className="block text-xs text-content-muted">Gender</label>
                <div className="flex gap-2">
                  {GENDER_OPTIONS.map(g => (
                    <button
                      ...
                      disabled={isSubmitting}
                      className={`rounded-control border px-3 py-1.5 text-sm font-medium transition ${
                        metadataGender === g
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1">
                  <label className="block text-xs text-content-muted">Color</label>
                  <input
                    type="text"
                    value={metadataColor}
                    onChange={e => setMetadataColor(e.target.value)}
                    className="w-full rounded-control border border-line-hairline bg-surface-inset p-2 text-content-primary text-sm focus:border-eth-blue focus:outline-none"
                    placeholder="Black"
                    disabled={isSubmitting}
                  />
                </div>
                <div className="space-y-1">
                  <label className="block text-xs text-content-muted">Style</label>
                  <input
                    type="text"
                    value={metadataStyle}
                ...
                <div className="space-y-1">
                  <label className="block text-xs text-content-muted">Size</label>
                  <input
                    type="text"
                    value={metadataSize}
            ...
            {metadataProgress && (
              <div className="flex items-center gap-3 rounded-control border border-eth-blue/30 bg-eth-blue/10 p-3">
                ...
                <p className="text-sm text-eth-blue-text">{metadataProgress}</p>
              </div>
            )}
        ...
        {activeTab === 'royalties' && (
          <div className="space-y-4">
            {/* Treasury share */}
            <div className="rounded-control border border-line-hairline bg-surface-inset/50 p-4">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-sm text-content-primary font-medium">Treasury Share</p>
                  <p className="text-xs text-content-faint">Remaining after royalty splits</p>
                </div>
                <p className="text-2xl font-mono text-eth-blue-text">{treasuryShare.toFixed(1)}%</p>
              </div>
            </div>

            {/* Current royalties */}
            <div className="space-y-2">
              <p className="text-sm text-content-muted">Current Royalties</p>
              {isLoadingRoyalties ? (
                <p className="text-xs text-content-faint">Loading...</p>
              ) : royalties.length === 0 ? (
                <p className="text-xs text-content-faint">No royalties configured</p>
              ) : (
                <div className="space-y-2">
                  {royalties.map((r, i) => (
                    <div key={i} className="flex items-center justify-between rounded-control border border-line-hairline bg-surface-inset p-3">
                      <span className="text-xs font-mono text-content-secondary">{r.recipient.slice(0, 10)}...{r.recipient.slice(-6)}</span>
                      <span className="text-sm font-mono text-eth-blue-text">{Number(r.percentage) / 100}%</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            {/* Add royalty form */}
            <div className="rounded-control border border-line-hairline bg-surface-inset/50 p-4 space-y-3">
              <p className="text-sm text-content-primary font-medium">Add Royalty</p>
              <div className="grid md:grid-cols-3 gap-3">
                <div className="md:col-span-2 space-y-1">
                  <label className="block text-xs text-content-muted">Recipient Address</label>
                  <input type="text" value={royaltyRecipient}
                    onChange={(e) => setRoyaltyRecipient(e.target.value)}
                    className="w-full rounded-control border border-line-hairline bg-surface-slab p-2 text-content-primary text-sm font-mono focus:border-eth-blue focus:outline-none"
                    placeholder="0x..." disabled={isSubmitting} />
                </div>
                <div className="space-y-1">
                  <label className="block text-xs text-content-muted">Percentage (%)</label>
                  <input type="number" min="0" max="100" step="0.01" value={royaltyPercentage}
                    onChange={(e) => setRoyaltyPercentage(e.target.value)}
                    className="w-full rounded-control border border-line-hairline bg-surface-slab p-2 text-content-primary text-sm focus:border-eth-blue focus:outline-none"
                    placeholder="5" disabled={isSubmitting} />
                </div>
              </div>
              <button onClick={handleAddRoyalty} disabled={isSubmitting || !canAdd}
                className="w-full rounded-control bg-eth-blue/10 border border-eth-blue/30 py-2 text-sm text-eth-blue-text hover:bg-eth-blue/20 transition disabled:opacity-50">
            ...
            {royalties.length > 0 && (
              <button onClick={handleClearRoyalties} disabled={isSubmitting || !canClear}
                className="w-full rounded-control border border-signal-reverted/30 bg-signal-reverted/10 py-2 text-sm text-signal-reverted hover:bg-signal-reverted/20 transition disabled:opacity-50">
        ...
        {activeTab === 'discounts' && (
          <div className="space-y-6">
            {/* POAP Discounts */}
            <div className="space-y-3">
              <p className="text-sm font-medium text-content-primary">POAP Discounts</p>
              {isLoadingPoapDiscounts ? (
                <p className="text-xs text-content-faint">Loading...</p>
              ) : poapDiscounts.length === 0 ? (
                <p className="text-xs text-content-faint">No POAP discounts configured</p>
              ) : (
                <div className="space-y-2">
                  {poapDiscounts.map((d, i) => (
                    <div key={i} className="flex items-center justify-between rounded-control border border-line-hairline bg-surface-inset p-3">
                      <div className="flex items-center gap-3">
                        <span className="text-xs font-mono text-content-secondary">Event #{d.eventId.toString()}</span>
                        <span className="text-sm font-mono text-eth-blue-text">{Number(d.discountBps) / 100}%</span>
                        <span className={`text-xs px-2 py-0.5 rounded-chip ${d.active ? 'bg-eth-blue/20 text-eth-blue-text' : 'bg-surface-ridge/20 text-content-faint'}`}>
                      ...
                      <button
                        ...
                        disabled={isSubmitting}
                        className="text-xs text-signal-reverted hover:text-signal-reverted disabled:opacity-50"
                      >
                        Remove
                      </button>
                    </div>
              ...
              <div className="rounded-control border border-line-hairline bg-surface-inset/50 p-4 space-y-3">
                <p className="text-sm text-content-primary font-medium">Add POAP Discount</p>
                <div className="grid md:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="block text-xs text-content-muted">Event ID</label>
                  ...
                  <div className="space-y-1">
                    <label className="block text-xs text-content-muted">Discount (%)</label>
            ...
            {/* Holder Discounts */}
            <div className="space-y-3">
              <p className="text-sm font-medium text-content-primary">Holder Discounts</p>
              {isLoadingHolderDiscounts ? (
                <p className="text-xs text-content-faint">Loading...</p>
              ) : holderDiscounts.length === 0 ? (
                <p className="text-xs text-content-faint">No holder discounts configured</p>
              ) : (
                <div className="space-y-2">
                  {holderDiscounts.map((d, i) => (
                    <div key={i} className="flex items-center justify-between rounded-control border border-line-hairline bg-surface-inset p-3">
                      <div className="flex items-center gap-3">
                        <span className="text-xs font-mono text-content-secondary">{d.token.slice(0, 10)}...{d.token.slice(-6)}</span>
                        <span className="text-sm font-mono text-eth-blue-text">
              ...
              <div className="rounded-control border border-line-hairline bg-surface-inset/50 p-4 space-y-3">
                <p className="text-sm text-content-primary font-medium">Add Holder Discount</p>
                <div className="space-y-3">
                  <div className="space-y-1">
                    <label className="block text-xs text-content-muted">Token Address</label>
                  ...
                  <div className="grid md:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="block text-xs text-content-muted">Type</label>
                      <select
