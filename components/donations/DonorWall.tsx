    ...
    <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-4">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-slate-300">
          Donors <span className="text-slate-500">· {token.symbol}</span>
        </h3>
        {data?.source === 'chain' && (
          <span
      ...
      <ul className="space-y-2">
        {entries.map((entry, i) => (
          <li
            ...
            className="flex items-start justify-between gap-3 rounded-lg border border-slate-700/60 bg-slate-900/40 p-3"
          >
            <div className="min-w-0 flex-1">
              ...
              {entry.message && (
                <p className="mt-1 break-words text-xs italic text-slate-400">
                  “{entry.message}”
                </p>
              )}
            </div>
