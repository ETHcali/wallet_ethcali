...
frontend/        → Generated by `npm run sync:contracts` from ../scs-ethcali/frontend:
                   ...
                   clone and its chain), CONTRACTS_SOURCE.json ({ repo, commit, generatedAt })
...
**Ethereum mainnet only** (owner decision 2026-09-23). Base exists in the registry
for the ethcali.eth registrar and nothing else; there is no multi-chain UI — no
picker, no chain badges, no per-chain balance breakdown. Adding a chain is a
deliberate future change, not a flag. Celo, Optimism and Unichain are gone from
this app (their contracts stay deployed; the contracts repo documents them).

1. **One registry.** `config/chains.ts` exports `ETHEREUM` (= `DEFAULT_CHAIN`) and
   `ENS_CHAIN` (Base), each with RPC, explorer, tokens and the `contracts` derived
   from the generated `frontend/addresses.json` and `frontend/swag-collection.json`.
   It is the only place a chain id, RPC, explorer, token address or contract address
   is written down. `config/constants.ts` keeps non-chain constants and a few
   compatibility views (`getRpcUrl`, `EXPLORER_URLS`, `NATIVE_SYMBOLS`) for
...
2. **Every read takes an explicit chain id.** `publicClientFor(id)` gives a cached
   viem client built from the registry's RPC, never from the wallet's provider.
   Pages pass `DEFAULT_CHAIN.id` (or `SWAG_COLLECTION.chainId`, `DONATION_CHAIN_ID`,
   `ENS_CONFIG.chainId`) down to every hook; nothing reads the wallet's network.
3. **The wallet is switched lazily, right before signing.** `useRequireChain(chainId)`
   ...
   `PRIVY_SUPPORTED_CHAINS` (Ethereum + Base) and `defaultChain: Ethereum`; Base is
   in that list only because `switchChain` throws for a chain outside it.
4. **No silent fallback.** `getChain(id)` returns `undefined` for an unsupported id
   and the UI says so. Admin visibility (`useAdminRoles`) reads the Ethereum contracts.
5. **ENS is the one exception.** `<label>.ethcali.eth` is minted through the Durin
   registrar on Base. `components/ens/ENSSection.tsx` calls
   `useRequireChain(ENS_CONFIG.chainId)` inside the claim button — one click switches
   the wallet to Base, then registers — and says so under the button ("Names live on
   Base; we switch you there to sign."). Everything else stays on Ethereum.
...
### Swag1155
- ERC-1155 for physical merch. **Ethereum mainnet**, one live clone: `ETHCALI-SWAG-2026`
  at `0x5a1012486764c217a20B5D1b97508E1de83179E7` (`SWAG_COLLECTION` in
  `config/constants.ts`, sourced from `frontend/swag-collection.json`, which
  `scripts/sync-contracts.mjs` writes from `deployments/<SWAG_CHAIN>-latest.json`,
  default `ethereum`). The Base clone `0xF602…5442` is paused and superseded; the
  `swag_variants` rows for 8453 stay in Supabase and `useSwagCatalogue` ignores them
  by `(chain_id, collection_address)`. Nothing in swag code names a chain except
  through `SWAG_COLLECTION.chainId` / `SWAG_CHAIN` (`hooks/swag/client.ts`). ENS
  stays on Base. Clone-only — a directly deployed `Swag1155` can never be initialized.
...
- User flow: approve exact `price × qty` USDC → `buy(tokenId, qty, paymentToken)`. Sponsored via Privy,
  always `{ chainId: SWAG.chainId, sponsor: true }`. Custom errors (`SoldOut`, `PaymentTokenNotAccepted`,
...
- Card and event orders mint through a **claim voucher**: server signs `ClaimVoucher{tokenId, to, quantity,
  orderRef, deadline}` (EIP-712 domain `ETHCaliSwag` v1, chainId 1 — from the json, verified against the
  collection's `eip712Domain()` by `scripts/swag-voucher-selftest.mjs`), user sends `claim(voucher, sig)`; `canClaim` mirrors it;
...
Minted on Base through the Durin registrar — the one action this Ethereum-only app signs on
another chain (see § Chains, point 5). The registry is an ERC-721 whose token id is the
